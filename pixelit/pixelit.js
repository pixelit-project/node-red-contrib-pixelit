        ...
        this.on('input', function (msg) {
            ...
            if (msg.topic === "screen_data_update") {
                if (msg.screenName !== undefined && msg.duration !== undefined) {
                    ...
                    node.status({ fill: "blue", shape: "ring", text: "Screen [" + msg.screenName + "] stored" });
                }
                else {
                    node.status({ fill: "red", shape: "dot", text: "Screen data cannot store!" });
                }
            }

            // Check is SleepMode Aktiv?! (Ab hier ist dann schluß! :-) )
            if (sleepModeAktiv == true && msg.sleepMode == null) {
                node.status({ fill: "yellow", shape: "ring", text: "Sleep Mode Aktiv!" });
                return;
            }

            // This topic sent by all providers to update the data in the context
            if (msg.topic === "matrix_control") {
                node.status({ fill: "grey", shape: "ring", text: "Matrix Control send" });
            ...
            if (msg.topic === "matrix_config") {
                node.status({ fill: "grey", shape: "ring", text: "Matrix Config send" });
            ...
            if (msg.topic === "playlist_update") {
                if ("undefined" !== typeof (msg.payload[0]["screenName"])) {
                    ...
                    node.status({ fill: "green", shape: "ring", text: "New Playlist stored" });

                    context.set("nowPlay", 0);
                    GetNextScreen();
                }
                else {
                    node.status({ fill: "red", shape: "dot", text: "New Playlist cannot store!" });
                }
            }
            ...
            function GetNextScreen() {
                ...
                if (playlist) {
                    ...
                    if (playlistCount > 0) {
                        ...
                        node.status({ fill: "green", shape: "ring", text: "Displaying [" + play.screenName + "] now" });
            ...
            function SendToPixelItConfig(myjson) {
                ...
                checkConnection(config.ip).then(function () {
                    ...
                    node.status({ fill: "yellow", shape: "dot", text: "Cannot send.." });
                });
            }
            ...
            function GetBitMap(input) {
                if (input.includes(",")) {
                    ...
                    if (bmp != null) {
                        ...
                        try {
                            var res = syncRequest('GET', 'https://api.bastelbunker.de/PixelItService/GetBMPByID/' + input, { headers: { 'User-Agent': 'Node_Red_Core' }, timeout: 1000 });
                        ...
                        if (body == null || body.id == null || body.id == 0) {
                            webBmp = "[64512,0,0,0,0,0,0,64512,0,64512,0,0,0,0,64512,0,0,0,64512,0,0,64512,0,0,0,0,0,64512,64512,0,0,0,0,0,0,64512,64512,0,0,0,0,0,64512,0,0,64512,0,0,0,64512,0,0,0,0,64512,0,64512,0,0,0,0,0,0,64512]";
                        }
                        else {
